...
class Baseball():
    ...
    def all_game(self):
        for n in range(len(self.home_teams)):
            if self.innings[n].text == '試合中止':
                self.home_scores.append(0)
                self.away_scores.append(0)
            if self.innings[n].text == '見どころ':
                self.home_scores.append('')
                self.away_scores.append(0)
        return self.date, self.home_teams, self.away_teams, self.home_scores, self.away_scores, self.innings
